...
func claimName(claim ClaimAttempt, client *fasthttp.Client) {
	...
	// Record request for VPN rotation
	if claim.Claim.Rotator != nil {
		claim.Claim.Rotator.RecordRequest(status, after.Sub(before))

		if claim.Claim.Rotator.ShouldRotate() {
			if err := claim.Claim.Rotator.Rotate(); err != nil {
				log.Log("err", "VPN rotate failed: %v", err)
			} else {
				log.Log("info", "rotated VPN to: %s", claim.Claim.Rotator.CurrentRegion())
			}
		}
	}
